...
describe('Test parks-serializer', () => {
  ...
  it('serializeParks should form a multiple JSON result as defined in openapi', () => {
    ...
    const structuredRawParks = [];
    _.forEach(structuredParks(rawParks), (value) => {
      structuredRawParks.push(_.omit(value, 'owner')); // remove relationship
    });
    const relationships = [
      {
        relationshipType: 'owner',
      ...
      },
      {
        relationshipType: 'owner',
        relationshipFields: {
          id: rawParks[1].ownerId,
          type: 'owner',
        },
      },
    ];
    testMultipleResources(serializedParks, structuredRawParks, resourceType, 'id', relationships);
